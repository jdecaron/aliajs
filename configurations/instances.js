    ...
    "services": [
      {
        ...
        "operations": {
          "initial": [
            ...
            { command: `sudo docker run  --detach --name vaultwarden --env DOMAIN="https://sauce-production.${process.env.ALIAJS_DEFAULT_TOP_LEVEL_DOMAIN}" --env LOGIN_RATELIMIT_MAX_BURST=20 --volume /vw-data/:/data/ --restart unless-stopped --publish 127.0.0.1:8000:80 vaultwarden/server:latest`, target: "new" },
          ],
          "backup": [
            { command: async ({ c }) => {
