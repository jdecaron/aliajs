...
ec2.describeImages(info.params.describeImages).promise()
  .then(async ({ Images }) => {
    ...
    Images.map(async (image) => {
      ...
      try {
        ...
        log.error({ error, info, message: `Could not delete snapshot for image ${image?.ImageId}`, slack: 'operations' })
      }
    })
  ...
  .catch((error) => {
    log.error({ error, info, message: 'Error creating new instance image', slack: 'operations' })
  })
